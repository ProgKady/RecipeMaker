...
public class CopyARecipePathController implements Initializable {
  ...
  @FXML
    ...
    @FXML
    void mady (Event event) {
      
        
        String hhd=model.getSelectionModel().getSelectedItem().toString();
        
        String ssaa=des.getText();
        
      //  des.clear();
        
     //   des.setText(ssaa+"\\"+hhd);
  ...
  /////////////////////////////////////////////////////////////////////////
    ...
    String stog = parts[parts.length - 3]; // "PILOT"
    String modw = parts[parts.length - 2];    // "HM"
    
    String fileName = file.getName();
    String lpq=pathy.replace("\\"+fileName,"").replace(stog,"PRODUCTION");
    des.setText(lpq);
    
    stageaction(null); // simulate ComboBox action
    stage.show(); // open the dropdown
    stage.getSelectionModel().select("PRODUCTION");
    stage.hide();
    
    modelaction(null); // simulate ComboBox action
    model.show(); // open the dropdown
    model.getSelectionModel().select(modw);
    model.hide();
      ...
      noti.showError();
        ...
        //////////////////////////////////////////////////////////////////////////
         ...
         String sqla = "INSERT INTO notifications (Recipient, Sender, Message, Delivered) VALUES (?, ?, ?, 0)";
          ...
          pst.setString(3, "We have copied "+filenamet+" successfully in "+value1+" of "+modell+" model in "+stagee+" from Pilots.");
          pst.executeUpdate();
          }
          catch (Exception e) {
          } finally {
